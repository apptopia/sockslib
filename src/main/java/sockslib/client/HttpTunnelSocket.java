...
/**
 * The class <code>HttpTunnelSocket</code> is proxy class that help developers use {@link HttpTunnelProxy} as
 * same as a java.net.Socket.<br>
 * For example:<br>
 * <pre>
 * HttpTunnelProxy proxy = new HttpTunnelProxy(new InetSocketAddress(&quot;127.0.0.1&quot;, 1080));
 * // Setting proxy...
 * Socket socket = new HttpTunnelSocket(proxy, new InetSocketAddress(&quot;whois.internic.net&quot;,
 * 43));
...
public class HttpTunnelSocket extends Socket {
  ...
  protected static final Logger logger = LoggerFactory.getLogger(HttpTunnelSocket.class);

  private HttpTunnelProxy proxy;
  ...
  /**
   * Socket that will connect to HTTP server.
   */
  private Socket proxySocket;

  /**
   * Create a socket and connect HTTP server.
   *
   * @param proxy            HTTP tunnel proxy.
   ...
   */
  public HttpTunnelSocket(HttpTunnelProxy proxy, String remoteServerHost, int remoteServerPort) throws IOException {
  ...
  /**
   * Same as {@link #HttpTunnelSocket(HttpTunnelProxy, String, int)}
   *
   * @param proxy   HTTP tunnel proxy.
   ...
   */
  public HttpTunnelSocket(HttpTunnelProxy proxy, InetAddress address, int port) throws IOException {
    this(proxy, new InetSocketAddress(address, port));
  }

  public HttpTunnelSocket(HttpTunnelProxy proxy, SocketAddress socketAddress) throws IOException {
   ...
   */
  public HttpTunnelSocket(HttpTunnelProxy proxy) throws IOException {
    this(proxy, proxy.createProxySocket());
  }

  /**
   * Creates a HttpTunnelSocket instance with a {@link HttpTunnelProxy} and a
   *
   * @param proxy       HTTP tunnel proxy.
   * @param proxySocket a unconnected socket. it will connect HTTP server later.
   */
  public HttpTunnelSocket(HttpTunnelProxy proxy, Socket proxySocket) {
    checkNotNull(proxy, "Argument [proxy] may not be null");
   ...
   *
   * @throws SocketException If a HTTP protocol error occurred.
   * @throws IOException     If an I/O error occurred.
   */
  private void initProxyChain() throws SocketException, IOException {
    List<HttpTunnelProxy> proxyChain = new ArrayList<HttpTunnelProxy>();
    HttpTunnelProxy temp = proxy;
    ...
    if (proxyChain.size() > 0) {
      HttpTunnelProxy pre = proxy;
      for (int i = 0; i < proxyChain.size(); i++) {
        HttpTunnelProxy chain = proxyChain.get(i);
  ...
  /**
   * Connect to HTTP server and server will proxy remote server.
   *
